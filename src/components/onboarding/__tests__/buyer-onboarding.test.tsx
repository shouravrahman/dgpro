...
describe('BuyerOnboarding', () => {
  ...
  it('should render step 1 initially', () => {
    render(<BuyerOnboarding {...mockProps} />);

    expect(
      screen.getByText('Tell us about your interests')
    ).toBeInTheDocument();
  ...
  it('should render step 2 when current step is 2', () => {
    ...
    render(<BuyerOnboarding {...step2Props} />);
  ...
  it('should render step 3 when current step is 3', () => {
    ...
    render(<BuyerOnboarding {...step3Props} />);
  ...
  it('should complete onboarding on final step submission', async () => {
    const step3Props = { ...mockProps, currentStep: 3, completedSteps: [1, 2] };
    ...
    await waitFor(() => {
      expect(mockOnStepComplete).toHaveBeenCalledWith(3, expect.any(Object));
    });
  });
});
